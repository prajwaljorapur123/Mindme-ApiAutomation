...
public class TeamEndPoints {
	...
	public static Response CreateTeam(String token, Team payload) {
		...
		Response response = given().filter(new AllureRestAssured()).headers("Authorization", "Bearer " + token)
				.contentType(ContentType.JSON).body(payload).when().post(Routes.teams_post_url);
	...
	public static Response TeamDetails(String token, int No, int size) {
		...
		Response response = given().filter(new AllureRestAssured()).headers("Authorization", "Bearer " + token)
				.queryParam("pageNo", No).queryParam("pageSize", size).when().get(Routes.teams_get_url);
	...
	public static Response TeamUpdate(String token, int teamId, Team payload) {
		...
		Response response = given().filter(new AllureRestAssured()).headers("Authorization", "Bearer " + token)
				...
				.body(payload).when().post(Routes.teams_update_url);

		return response;

	}
	
	public static Response TeamEmployees(String token, int teamId) {

		Response response = given().filter(new AllureRestAssured()).headers("Authorization", "Bearer " + token)
				.queryParam("teamId", teamId)

				.when().get(Routes.teams_employees);

		return response;

	}
	
	public static Response DeleteTeamMember(String token,int memberId, int teamId) {

		Response response = given().filter(new AllureRestAssured()).headers("Authorization", "Bearer " + token)
				.queryParam("memberId", memberId)
				.queryParam("teamId", teamId)

				.when().get(Routes.delete_team_member);
